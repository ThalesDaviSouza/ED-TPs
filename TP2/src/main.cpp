...
int main(){
  ...
  Pacote teste3(3, 10, "Teste3.5", "Teste1000", None, 0, 5);
  ...
  Heap<Pacote> temp = Heap<Pacote>(10);
  HeapItem<Pacote> t = HeapItem<Pacote>(0, &teste); 
  HeapItem<Pacote> t1 = HeapItem<Pacote>(1, &teste1); 
  HeapItem<Pacote> t2 = HeapItem<Pacote>(2, &teste2); 
  HeapItem<Pacote> t3 = HeapItem<Pacote>(3, &teste3); 
  ...
  while(!temp.Vazio()){
    HeapItem<Pacote> aux = temp.Remover();
    cout << "Chave: "<< aux.chave << " Id: " << aux.value->id << " Remetente: " << aux.value->nomeRemetente << endl;
  }
